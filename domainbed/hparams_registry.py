...
def _hparams(algorithm, dataset, random_seed):
    ...
    _hparam('resnet18', True, lambda r: True)   # TODO: FOR decaug
    ...
    if algorithm in ['DANN', 'CDANN']:
        ...
        # _hparam('lambda', 0.8, lambda r:r.uniform(0.75, 0.95))
        # _hparam('multi',10, lambda r: r.choice(list(range(8, 10, 2))))
        # _hparam('alpha', 0.5, lambda r: r.uniform(0.4, 0.6))
        # _hparam('kl_w', 8e-4, lambda r:r.choice([8e-4, 1e-5, 5e-5]))
        # _hparam('recon_w', 1, lambda r:r.choice([1, 0.8, 0.5]))
        _hparam('lambda', 0.8, lambda r:r.uniform(0.75, 0.95))
        _hparam('multi',10, lambda r: r.choice(list(range(8, 10, 2))))
        _hparam('alpha', 0.5, lambda r: r.uniform(0.4, 0.6))
        _hparam('kl_w', 8e-4, lambda r:r.choice([8e-4]))
        _hparam('recon_w', 1, lambda r:r.choice([1, 0.85, 0.75]))

    elif algorithm == 'DecAug':
        _hparam('perturbation', 1, lambda r:r.choice([1]))
        _hparam('balance1', 0.1, lambda r: r.choice([0.1]))
        _hparam('balance2', 0.1, lambda  r:r.choice([0.1]))
        _hparam('balanceorth', 0.01, lambda r:r.choice([0.01]))
        _hparam('epsilon', 1e-8, lambda r:r.choice([1e-8]))
        
    elif algorithm == 'DecOrth':
        _hparam('perturbation', 1, lambda r:r.choice([1]))
        _hparam('balance1', 0.1, lambda r: r.choice([0.1]))
        _hparam('balance2', 0.1, lambda  r:r.choice([0.1]))
        _hparam('balanceorth', 0.01, lambda r:r.choice([0.01]))
    ...
    if dataset in SMALL_IMAGES:
        ...
        _hparam('batch_size', 128, lambda r: int(2**r.uniform(5, 7)))
    else:
        # _hparam('batch_size', 32, lambda r: int(2**r.uniform(3, 5.5)))  # Changed from 32 to 64
        _hparam('batch_size', 32, lambda r: 64)
