...
def leaveone_env_combinations(n):
    assert(n >= 3)
    for i in range(n):
        # yield [i]
        for j in range(i+1, n):
...
def make_args_list(n_trials, dataset_names, algorithms, n_hparams_from, n_hparams, steps,
    data_dir, task, holdout_fraction, single_test_envs, hparams, leaveone):
    args_list = []
        ...
        for dataset in dataset_names:
            for algorithm in algorithms:
                if single_test_envs:
                    ...
                    all_test_envs = leaveone_env_combinations(
...
if __name__ == "__main__":
    ...
    parser.add_argument('--leaveone', action='store_true')
    ...
    if args.command == 'launch':
        ...
        # TODO delete speicial dataset
        # to_delete = [j for j in jobs if job.train_args['dataset'] == 'DomainNet']
        # to_delete = []
        # for job in jobs:
        #     if job.train_args['dataset'] == 'DomainNet':
        #         to_delete.append(job)
        #         print(job.train_args['dataset'])
